...
{
    public class Task_Server:IBaseTask
    {
        #region parameter
        ...
        private void Transition(WORK target)
        {
            ...
            {
                tool.SaveHistoryToFile("[Task](Task_Server)" + target.ToString());
                UpdateTaskState("[Task](Task_Server)" + target.ToString());
            }
        ...
        public Task_Server(string set_state = "Default")
        {
            switch (set_state)
            {
                case "Default":
        ...
        private void WaitClient()
        {
            ...
            {
                tCPIP_Server.WaitClientCommand();
            }
        }
        ...
        private void RunLoop(TASK_STATUS task_status)
        {
            ...
            switch (state)
            {
                case WORK.INITIAL:
                    {
                        ...
                        tCPIP_Server.Open("127.0.0.1", 87); //開啟Server

                        Task task = Task.Run(() => WaitClient());   //等待Client下命令執行緒
                ...
                case WORK.IDLE:
                    {
                        if(tCPIP_Server.GetMessage() == "WaferAlign")
                        {
                ...
                case WORK.WAFER_ALIGN:
                    {
                        Calculate = new SubTask_AngleCalculate("ServerMode");
                ...
                case WORK.SUCCESS:
                    {
                        tCPIP_Server.SendMessage(Scope.WaferAngle.ToString());
                        Transition(WORK.IDLE);
                    }
                    break;
                case WORK.FAIL:
