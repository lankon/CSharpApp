...
{
    public class TemperatureController_TPT8000	//20230815 Pohan Start
    {
        ...
        int TempValue1 = 200, TempValue2 = 180, TempValue3 = 150, TempValue4 = 120, TempValue5 = 85;
        Tool tool = new Tool();
        #endregion
        ...
        public bool Open(String com, String baudrate, String parity)
        {
            ...
            if (!Comport.IsOpen)
            {
                try
                {
                    ...
                    tool.SaveHistoryToFile($"PortName = {com}");
                    tool.SaveHistoryToFile($"PortName = {com}");
                    tool.SaveHistoryToFile($"BaudRate = {baudrate}");
                    tool.SaveHistoryToFile($"Parity = {parity}");
                    tool.SaveHistoryToFile($"---Comport Open Success---");
                }
                catch (UnauthorizedAccessException ex)
                {
                    tool.SaveHistoryToFile($"---Comport Open Fail---");
                    res = false;
                }
        ...
        public bool Close()
        {
            ...
            if (Comport.IsOpen)
            {
                try
                {
                    ...
                    tool.SaveHistoryToFile($"---Comport Close Success---");
                }
                catch (Exception ex)
                {
                    tool.SaveHistoryToFile($"---Comport Close Fail---");
                    res = false;
                }
            }
        ...
        public bool SetValue(int value) 
        {
            try
            {
                ...
                tool.SaveHistoryToFile($"SetValue sv_value: {sv_value}");
            ...
            {
                tool.SaveHistoryToFile($"SetValue exception: {ex}");
                return false;
            }
        }
        public bool AskPV()
        ...
        public String GetAns() 
        {
            ...
            {                
                ...
                tool.SaveHistoryToFile($"Receive ComportString {FivePointValue}");
                temp = receive_msg;
                ...
                if (receive_msg.Contains("\r"))
                {
                    ...
                    tool.SaveHistoryToFile($"Calculate Offset_Value{Offset_Value}");
                    ...
                    tool.SaveHistoryToFile($"TPT Receive Present Value{temp}");
                    return temp;
                }
                else
                {
            ...
            {
                tool.SaveHistoryToFile($"TPT GetAns exception: {ex}");
                return "error";
            }
        }        
        ...
        public bool CheckEndCode() 
        {
            ...
            try
            {
                ...
                {
                    receive_msg += "\n";
                    tool.SaveHistoryToFile(receive_msg);
                    res = true;
                }
            }
            catch (Exception ex)
            {
                tool.SaveHistoryToFile($"TPT CheckEndCode exception: {ex}");
            }
            return res;
        }
        ...
        {  
            ...
            try
            {
                ...
                {
                    tool.SaveHistoryToFile($"Set Temperature Fail");
                }
            }
            catch (Exception ex)
            {
                tool.SaveHistoryToFile($"Set Temperature Success: {ex}");
            }
            return res;
        }
        public bool Stop()
        ...
        public bool SetPVOffset(int set_temp_value, int high_temp, int low_temp, double high_comp, double low_comp)
        {
            try
            {
                ...
                tool.SaveHistoryToFile($"SetPVOffset offset_value: {offset_value}");
            ...
            {
                tool.SaveHistoryToFile($"SetPVOffset exception: {ex}");
                return false;
            }
        }
        public bool AskPID()
        ...
        bool Write(byte[] buffer, int offset, int count)
        {
            ...
            if (Comport.IsOpen)
            {
                ...
                {
                    tool.SaveHistoryToFile($"Write exception: {ex}");
                }
            }
            else
            {
                tool.SaveHistoryToFile($"Comport is not open");
            }
        ...
        byte[] Read(int expect_read_length)
        {
            ...
            if (Comport.IsOpen)
            {
                ...
                {
                    received_buf = null;
                    tool.SaveHistoryToFile($"Read exception: {ex}");
                }
            }
            else
            {
        ...
        public bool DoandConfirmHeatOperation_FLCR()
        {
            ...
            try
            {
                ...
                {
                    tool.SaveHistoryToFile($"Confirm Heat Operation Fail");
                }
            }
            catch (Exception ex)
            {
                tool.SaveHistoryToFile($"Confirm Heat Operation Success: {ex}");
            }            
            return res;
        }
        ...
        void DiscardOutBuffer()
        {
            if (Comport.IsOpen)
            {
                ...
                {
                    tool.SaveHistoryToFile($"DiscardOutBuffer exception: {ex}");
                }
            }
        }
        void DiscarInBuffer()
        {
            if (Comport.IsOpen)
            {
                ...
                {
                    tool.SaveHistoryToFile($"DiscardInBuffer exception: {ex}");
                }
            }
        }
        void Clear()
        ...
        byte[] GetAskCommand(ASK_ITEM item)
        {
            ...
            {
                tool.SaveHistoryToFile($"Get{item}Command exception: {ex}");
                cmd = null;
            }
        ...
        bool Ask(ASK_ITEM item)
        {
            ...
            try
            {
                ...
                {
                    tool.SaveHistoryToFile($"Read Temperature Fail");
                }
            }
            catch (Exception ex)
            {
                tool.SaveHistoryToFile($"Read Temperature Success: {ex}");
            }            
            return res;
        }
